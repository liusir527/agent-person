...
def _resolve_workspace_root():
    ...
    该函数与 deploy-build / vpp-api-sync / mr-merge 三个脚本的解析逻辑一致。
    """
...
# 与 deploy-build 约定一致（AGENT_ASSETS_DIR 环境变量可覆盖）。
# 原实现由 SCRIPT_DIR 上溯推导，脚本被复制/重装后易漂移；改为 git 探测后跟随仓库搬迁。
AGENT_ASSETS_DIR = _resolve_workspace_root()
CERT_DIR = os.path.normpath(os.path.join(str(AGENT_ASSETS_DIR), ".dsh", "env_config", "certificate-apply"))
