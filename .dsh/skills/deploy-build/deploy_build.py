...
def _resolve_workspace_root():
    ...
    该函数与 mr-merge/mr_merge.py 的 _resolve_project_root() 行为一致（参数名不同）；
    所有运行时中间产物（config / sync 清单 / 日志）统一落到
    `<workspace_root>/.dsh/env_config/<skill>/`，不再随 cwd（NPP 代码目录）漂移。
    """
...
def build_parser():
    ...
    conn.add_argument("--project-dir", help="项目配置目录(含 .dsh/env_config), 默认 AGENT_ASSETS_DIR 环境变量 / git 根 / cwd 兜底 (见 _resolve_workspace_root())")
