...
# =====================================================================
#
# 项目根 = mr-state/ 报告/状态文件 的落盘基目录。解析优先级：
#   1. 环境变量 MR_PROJECT_ROOT（CI / 外部脚本显式指定）
#   2. `git rev-parse --show-toplevel`（在 cwd 或其父目录中探测到的 git 根）
#   3. `os.getcwd()` 兜底（脚本被复制到非 git 目录运行时），附 warning
#
# 这样可以：
#   - 跟随仓库搬迁而无需改代码
#   - 与 deploy-build / certificate-apply 等其他 skill 的"以 git 根为锚"语义一致
#   - 兼容脚本被复制调试的场景
# =====================================================================

def _resolve_project_root():
    """按优先级解析项目根：环境变量 → git 根 → cwd 兜底。"""
    # 1) 环境变量优先
    env_root = os.environ.get("MR_PROJECT_ROOT")
    if env_root:
        p = Path(env_root).expanduser().resolve()
        if p.is_dir():
            return p
        print(f"[WARN] MR_PROJECT_ROOT={env_root!r} 指向的目录不存在，回退到探测", file=sys.stderr)
    ...
    print(
        f"[WARN] 未探测到 git 根（cwd={cwd}），mr-state/ 将落到 cwd 下；"
        f"如需锚定到具体仓库，请设置环境变量 MR_PROJECT_ROOT",
        file=sys.stderr,
    )
    return cwd
