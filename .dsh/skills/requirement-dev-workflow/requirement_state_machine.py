...
脚本驱动的确定性状态机，落地 .dsh/hooks/AGENT.md 中「需求开发流程铁律」的规则：
主状态机 需求澄清→需求分析→方案评审→出实施计划→编码实现→测试验证→交付上线→结束，
每个主状态内含微观状态机；硬校验门禁 + state.json 三键断点。

主状态（含微观状态）：
  需求澄清    需求理解→范围界定→需求拆解→验收标准    退出产物 需求理解说明书.md
  需求分析    现状调研→数据流梳理→方案设计→影响评估    退出产物 需求设计说明书.md
  方案评审    评审→可验证性推演→判定→轮次记账          退出产物 评审结果-rN.md
  出实施计划  计划草拟→任务拆解→用户确认                退出产物 实施计划.md
  编码实现    修改→代码审查→编译通过→修复评审问题      退出产物 编译产物（deploy_build 验证，非脚本跟踪）
  测试验证    自测记录→受限环境测试执行→转人工验收→人工验收通过 退出产物 测试记录.md（含交接项登记）
  交付上线    生成MR描述→提交MR→人工review→合并部署→沉淀经验 退出产物 MR描述.md+交付说明.md
  结束        无

用法（仿 path_guard.py / deploy_build.py 惯例）：
  python requirement_state_machine.py init --req-id REQ-1001 --title "..." --module "安全策略"
  python requirement_state_machine.py state --req-id REQ-1001 [--json]
  python requirement_state_machine.py progress --req-id REQ-1001   # 可视化进度总览
  python requirement_state_machine.py micro --req-id REQ-1001 --to 需求理解
  python requirement_state_machine.py advance --req-id REQ-1001 --to 需求分析
  python requirement_state_machine.py review --req-id REQ-1001 --result PASS --round 1 --issues "..."
  python requirement_state_machine.py record --req-id REQ-1001 --phase analysis --minutes 30
  python requirement_state_machine.py jira --req-id REQ-1001 --flag fetched --value true
  python requirement_state_machine.py confirm --req-id REQ-1001 --flag requirement_confirmed --by <确认人> [--note]
  python requirement_state_machine.py gate --req-id REQ-1001 --flag build_passed --value true --evidence <编译日志>
  python requirement_state_machine.py gate-check --req-id REQ-1001
  python requirement_state_machine.py resolve --req-id REQ-1001
  python requirement_state_machine.py close --req-id REQ-1001 --conclusion 交付说明.md
  python requirement_state_machine.py reset --req-id REQ-1001
...
# 工作区根 = 本文件上级三级（skills/requirement-dev-workflow/ -> skills/ -> 仓库根）
WORKSPACE = Path(__file__).resolve().parent.parent.parent
SKILL_DIR = Path(__file__).resolve().parent
TEMPLATES_DIR = SKILL_DIR / "templates"
DEFAULT_STATE_DIR = WORKSPACE / "runtime/req"
...
# 主状态 → 微观状态（有序）
MICRO_STATES = {
    "需求澄清": ["需求理解", "范围界定", "需求拆解", "验收标准"],
    "需求分析": ["现状调研", "数据流梳理", "方案设计", "影响评估"],
    "方案评审": ["评审", "可验证性推演", "判定", "轮次记账"],
    "出实施计划": ["计划草拟", "任务拆解", "用户确认"],
    "编码实现": ["修改", "代码审查", "编译通过", "修复评审问题"],
    "测试验证": ["自测记录", "受限环境测试执行", "转人工验收", "人工验收通过"],
    "交付上线": ["生成MR描述", "提交MR", "人工review", "合并部署", "沉淀经验"],
    "结束": [],
}
...
ARTIFACT_FILES = {
    "需求澄清": ("analysis", "需求理解说明书.md"),
    "需求分析": ("design", "需求设计说明书.md"),
    "方案评审": ("review", "评审结果-r{round}.md"),
    "出实施计划": ("plan", "实施计划.md"),
    "测试验证": ("selftest", "测试记录.md"),
    "交付上线": ("pr_desc", "MR描述.md"),
}

# 评审 FAIL 回退的起始微观状态（回「需求分析」重新调研）
REVIEW_FAIL_BACK_TO = "现状调研"
...
def state_path(req_id: str, state_dir: Path | None = None) -> Path:
    base = state_dir or DEFAULT_STATE_DIR
    return base / req_id / "state.json"


def req_dir(req_id: str, state_dir: Path | None = None) -> Path:
    base = state_dir or DEFAULT_STATE_DIR
    return base / req_id


def load_state(req_id: str, state_dir: Path | None = None) -> dict | None:
    p = state_path(req_id, state_dir)
    if not p.exists():
        return None
    try:
...
def save_state(state: dict, state_dir: Path | None = None) -> None:
    ...
    p = state_path(state["req_id"], state_dir)
...
def render_progress(state: dict) -> str:
    ...
    lines = [f"进度总览  {state['req_id']}  {state['title'] or ''}".rstrip()]
...
def artifact_path(state: dict, key: str, state_dir: Path | None = None) -> Path | None:
    """artifacts 键 → 磁盘路径。空名返回 None；绝对路径原样返回；相对路径相对需求状态目录解析。"""
    ...
    if p.is_absolute():
        return p
    return req_dir(state["req_id"], state_dir) / name


def find_file_in_reqdir(reqid: str, state_dir: Path | None, name: str) -> Path:
    return req_dir(reqid, state_dir) / name
...
# init
# ---------------------------------------------------------------------------
def register_in_writable_dirs(req_dir: Path) -> None:
    """状态目录放行自检（配合 restricted 文件门控 + 运行时产物纪律）。

    - 默认状态目录在 `<workspace_root>/runtime/req/` 下：`runtime` 已在 .dsh/rules/dirs.json
      登记（L2 硬门禁前置，见 hooks/AGENT.md「运行时产物纪律」），无需额外登记；
    - 自定义 --dir 到 workspace 内其他路径：提示手动登记 dirs.json（restricted 下未登记目录
      文件工具一律 MISS 拒绝，无法落盘）；
    - 无论哪种情况，用 path_guard.py validate-path 自检落点是否符合运行时产物纪律
      （DEFAULT_STATE_DIR 必须 runtime/ 子目录），违规则显式告警。

    说明：早期版本调用 .dsh/tools/writable_dirs.py 登记，该工具已不存在；本函数不再依赖它。
    """
    tool = WORKSPACE / ".dsh" / "tools" / "path_guard.py"
    try:
        rel = req_dir.resolve().relative_to(WORKSPACE.resolve())
    except ValueError:
        print(f"  [登记跳过] 状态目录在 workspace 外，无需登记：{req_dir}")
        return
    # 落点合规自检（运行时产物纪律：首段目录必须 runtime/）
    r = subprocess.run(
        [sys.executable, str(tool), "validate-path", str(req_dir)],
        capture_output=True, text=True, encoding="utf-8", errors="replace",
        env={**os.environ, "PYTHONIOENCODING": "utf-8"},
    )
    out = (r.stdout or r.stderr or "").strip()
    if r.returncode != 0:
        print(f"  [落点警告] {out}\n    提示：状态目录应落在 <workspace_root>/runtime/<分类>/ 下（运行时产物纪律）；"
              f"restricted 模式下未登记 dirs.json 的目录文件工具无法落盘。")
        return
    if rel.parts and rel.parts[0] == "runtime":
        print(f"  [放行确认] {req_dir}（runtime/ 下已登记 .\\runtime，可直接落盘）")
    else:
        print(f"  [放行提示] {req_dir} 不在 runtime/ 下：restricted 模式下需手动把该目录追加进 "
              f".dsh/rules/dirs.json 的 dirs 数组，否则文件工具无法落盘。")


def cmd_init(args) -> int:
    req_id = args.req_id.strip()
    if not req_id:
        return fatal("--req-id 必填")
    if not re.match(r"^[A-Za-z0-9_.\-]+$", req_id):
        return fatal(f"--req-id 含非法字符：{req_id}（仅允许字母数字_.-）")
    # L3: 防路径穿越（. / .. 会解析到 workspace 根）
    if req_id in (".", "..") or req_id.endswith("/..") or req_id.endswith("/."):
        return fatal(f"--req-id 非法路径：{req_id}")

    base = args.dir if args.dir else DEFAULT_STATE_DIR
    req = req_dir(req_id, base)
    if req.exists():
        return fatal(f"目标目录已存在，禁止覆盖：{req}")
    if not TEMPLATES_DIR.is_dir():
        return fatal(f"模板目录不存在：{TEMPLATES_DIR}")

    req.mkdir(parents=True)
    register_in_writable_dirs(req)
    # templates/ 全部为产出物模板（需求理解说明书/需求设计说明书/评审结果/实施计划/测试记录/MR描述/交付说明），
    # 一律不预复制——agent 按 templates/ 结构在 req 目录创建产物，作为 advance/review/close 门禁依据。
    # 预复制成骨架会让产物门禁（按文件名查磁盘）恒过，违背硬校验设计。

    state = {
        "schema_version": 1,
        "req_id": req_id,
        "jira_key": args.jira_key or req_id,
        "title": args.title or "",
        ...
        "artifacts": {
            "analysis": ARTIFACT_FILES["需求澄清"][1],
            "review": "",
            "reviews": [],
            "plan": "",
            "selftest": "",
        ...
        "gates": {
            "requirement_confirmed": False,  # 需求理解+验收标准已与需求方对齐（仅 confirm 置位）
            "design_reviewed": False,        # 设计方案经对抗评审 PASS（仅 review 置位）
            "plan_confirmed": False,         # 实施计划经用户确认（仅 confirm 置位）
            "worktree_ready": False,         # 编译机隔离 worktree 已派生并登记（仅 worktree-set 置位）
            "code_reviewed": False,          # 代码改动经 code-reviewer 审查 PASS（证据型）
            "build_passed": False,           # 编译通过（证据型）
            "test_handed": False,            # 已转人工验收
            "test_passed": False,            # 人工验收通过（可测项全过+交接项登记，仅 confirm 置位）
            "mr_reviewed": False,            # MR 经人工 review（仅 confirm 置位）
            "merged": False,                 # 合并/部署上线完成（仅 confirm 置位）
        },
        "timeline": [],
        "review_rounds": [],
        "worklog": {
            "sync_mode": "local",
            "local": {
                "analysis": {"minutes": 0, "note": ""},   # 需求澄清+需求分析
                "review": {"minutes": 0, "note": ""},     # 方案评审
                "plan": {"minutes": 0, "note": ""},       # 出实施计划
                "coding": {"minutes": 0, "note": ""},     # 编码实现
                "test": {"minutes": 0, "note": ""},       # 测试验证
                "deliver": {"minutes": 0, "note": ""},    # 交付上线
            },
            "total_minutes": 0,
        ...
        "git": {
            "branch": "",
            "commit_sha": "",
            "commit_format": f"feat: {req_id} 【{args.module}】描述",
            "pr_url": "",
        },
    }
    append_timeline(state, "init", "需求澄清", "需求理解", "创建状态机")
    save_state(state, base)
    print(f"已初始化：{req}")
    print(f"  产物模板不预复制：按 .dsh/skills/requirement-dev-workflow/templates/ 结构创建产物文件（门禁按文件名校验）")
    print(f"  主状态：需求澄清 / 微观状态：需求理解")
...
def cmd_state(args) -> int:
    state = load_state(args.req_id, args.dir)
    if state is None:
        return fatal(f"状态文件不存在：{state_path(args.req_id, args.dir)}（先 init）")
    if args.json:
    ...
    print(f"需求: {state['req_id']}  {state['title']}")
    ...
    if state.get("worktree"):
        ...
        print("  worktree：未派生（进入【编码实现】前须 deploy_build --worktree feature-<需求单号>）")
...
def cmd_gate_check(args) -> int:
    ...
    for cur, (_, name) in ARTIFACT_FILES.items():
        if cur == "方案评审":
            continue  # 评审产物由 review 命令落盘，按最新轮次文件（不在此列）
        if MAIN_STATES.index(cur) >= MAIN_STATES.index(ms):
            continue  # 当前及之后状态的产物未到产出时点
        p = find_file_in_reqdir(reqid, args.dir, name)
...
def cmd_resolve(args) -> int:
    ...
    if state.get("escalated"):
        ...
        return 0
    if ms == "需求澄清":
        print(f"下一步：完成「{mis}」后推进微观状态；产出需求理解说明书.md（含验收标准）→ 请需求方 confirm requirement_confirmed → advance 到 需求分析。")
    elif ms == "需求分析":
        print(f"下一步：完成「{mis}」后推进微观状态；产出需求设计说明书.md 后 advance 到 方案评审。")
    elif ms == "方案评审":
        print(f"下一步：当前第 {state['review_round']} 轮已完成；若为 FAIL 且轮次<3 已回 需求分析，重新调研后再审；若为 PASS 已推进 出实施计划。")
    elif ms == "出实施计划":
        print("下一步：确认实施计划.md 后询问用户确认（置 gates.plan_confirmed），再 advance 到 编码实现。")
        wt = state.get("worktree")
        if wt:
            print(f"  worktree 已登记：{wt.get('name')}（{wt.get('remote_dir')}）")
        else:
            print("  ⚠ worktree 尚未派生：进入 编码实现 前须先在编译机执行 deploy_build --worktree feature-<需求单号>（派生成功自动登记）")
    elif ms == "编码实现":
        print("下一步：完成代码审查（code_reviewed）+ 编译通过（build_passed）后 advance 到 测试验证。")
    elif ms == "测试验证":
        print("下一步：完成测试记录.md（含交接项登记）+ 人工验收通过（test_passed）后 advance 到 交付上线。")
    elif ms == "交付上线":
        print("下一步：生成MR描述.md + 提交MR + 人工review（mr_reviewed）+ 合并部署（merged）+ 沉淀经验（memory-gen 生成经验文档并更新索引、memory-push 推送 git，然后 micro --to 沉淀经验）后 close。")
    else:
        print("已结束。")
    return 0
...
def gate_prereq(next_main: str, state: dict, state_dir: Path | None) -> list[str]:
    ...
    reqid = state["req_id"]
    if next_main == "需求分析":
        p = find_file_in_reqdir(reqid, state_dir, ARTIFACT_FILES["需求澄清"][1])
        if not check_file_exists(p):
            missing.append("需求理解说明书.md 不存在或为空")
        if not state["gates"].get("requirement_confirmed"):
            missing.append("gates.requirement_confirmed 未置位（需求理解+验收标准未获需求方确认）")
    elif next_main == "方案评审":
        p = find_file_in_reqdir(reqid, state_dir, ARTIFACT_FILES["需求分析"][1])
        if not check_file_exists(p):
            missing.append("需求设计说明书.md 不存在或为空")
    elif next_main == "出实施计划":
        if not state["gates"].get("design_reviewed"):
            missing.append("gates.design_reviewed 未置位（设计方案未经对抗评审 PASS）")
    elif next_main == "编码实现":
        p = find_file_in_reqdir(reqid, state_dir, ARTIFACT_FILES["出实施计划"][1])
        if not check_file_exists(p):
            missing.append("实施计划.md 不存在")
        if not state["gates"].get("plan_confirmed"):
            missing.append("gates.plan_confirmed 未置位（用户未确认实施计划）")
        if not state["gates"].get("worktree_ready"):
            missing.append("gates.worktree_ready 未置位（编译机 worktree 未派生/登记，先执行 deploy_build --worktree feature-<需求单号>）")
    elif next_main == "测试验证":
        ...
        p = find_file_in_reqdir(reqid, state_dir, ARTIFACT_FILES["测试验证"][1])
        if not check_file_exists(p):
            missing.append("测试记录.md 不存在")
        if not state["gates"].get("test_passed"):
            missing.append("gates.test_passed 未置位（人工验收未通过，须含受限环境可测项全过+交接项登记）")
    elif next_main == "结束":
        if not state["gates"].get("mr_reviewed"):
            missing.append("gates.mr_reviewed 未置位（MR 未经人工 review）")
        if not state["gates"].get("merged"):
            missing.append("gates.merged 未置位（未确认合并/部署上线）")
        if state["micro_state"] != "沉淀经验":
            missing.append("微观状态未推进到「沉淀经验」（需求开发流程结束前必须完成经验沉淀：memory-gen 生成经验文档 + 更新索引，memory-push 推送 git，然后 micro --to 沉淀经验）")
        p = artifact_path(state, "conclusion", state_dir)
        if not check_file_exists(p):
            missing.append("交付说明.md 不存在（close 需 --conclusion 传交付说明.md 或终稿 XX需求-REQ-SEQ需求实现说明.md）")
    return missing
...
def cmd_advance(args) -> int:
    ...
    # 落退出产物路径（方案评审的产物由 review 命令写，此处只登记文件名占位）
    art_key, art_name = ARTIFACT_FILES.get(cur, (None, None))
    if art_key and cur != "方案评审":
...
# review（对抗评审轮次）
# ---------------------------------------------------------------------------
PASS_MARK = re.compile(r"可推演[:：]?\s*(PASS|成功)")
...
def cmd_review(args) -> int:
    ...
    if guard is not None:
        return guard
    # M1: 仅「方案评审」或「需求分析」（FAIL 回退态）受理 review，防 PASS 后被误用回滚
    if state["main_state"] not in ("方案评审", "需求分析"):
        return gate_reject(
            f"当前主状态「{state['main_state']}」不允许执行 review（仅「方案评审」/「需求分析」可评审）"
        )
    ...
    # 评审对象产物必须存在
    ap = artifact_path(state, "design", args.dir)
    if not check_file_exists(ap):
        return gate_reject("需求设计说明书.md 不存在或为空，无法评审")

    # PASS 时硬校验产物含可推演标记（防自报 PASS 无产物）
    art_name = ARTIFACT_FILES["方案评审"][1].format(round=args.round)
    artifact_file = find_file_in_reqdir(state["req_id"], args.dir, art_name)
    if result == "PASS":
        if not check_file_exists(artifact_file):
            return gate_reject(f"评审产物 {art_name} 不存在；PASS 必须由评审 agent 落盘该文件")
        content = artifact_file.read_text(encoding="utf-8", errors="replace")
        if not PASS_MARK.search(content):
            return gate_reject(f"评审产物 {art_name} 不含「可推演：PASS/成功」标记，禁止 PASS")
    ...
    if result == "FAIL":
        if args.round < MAX_REVIEW_ROUNDS:
            # 回需求分析，重新调研
            state["main_state"] = "需求分析"
            state["micro_state"] = REVIEW_FAIL_BACK_TO
            append_timeline(state, "review_fail", "需求分析", REVIEW_FAIL_BACK_TO,
                            f"第{args.round}轮 FAIL：{args.issues}")
            print(f"第{args.round}轮评审 FAIL → 主状态回「需求分析」（微观：{REVIEW_FAIL_BACK_TO}）")
            print(f"  补充方向：{args.issues or '（见 评审结果-r%d.md）' % args.round}")
        else:
            ...
            print(f"第{args.round}轮评审 FAIL（已达上限）→ 转人工接管，停止自动推进")
    else:
        state["gates"]["design_reviewed"] = True
        state["main_state"] = "出实施计划"
        state["micro_state"] = FIRST_MICRO["出实施计划"]
        append_timeline(state, "review_pass", "出实施计划", FIRST_MICRO["出实施计划"],
                        f"第{args.round}轮 PASS，可推演通过")
        print(f"第{args.round}轮评审 PASS → gates.design_reviewed 置位，推进到「出实施计划」")
...
# record / jira
# ---------------------------------------------------------------------------
PHASES = ("analysis", "review", "plan", "coding", "test", "deliver")
...
# 禁止用 gate 命令手拍 true 绕过"需求方确认/用户确认/人工验收/人工review/人工确认合并部署"。
HUMAN_CONFIRM_GATES = {"requirement_confirmed", "plan_confirmed", "test_passed", "mr_reviewed", "merged"}
# 证据型门禁：置位须带 --evidence <存在非空文件>（编译日志/代码审查报告路径），
# 防止仅凭口头声称置位。code_reviewed 可用审查报告，build_passed 可用编译日志。
...
def cmd_gate(args) -> int:
    ...
    if flag == "worktree_ready":
        return gate_reject(
            ...
            "deploy_build --worktree feature-<需求单号> 派生隔离 worktree（派生成功自动登记置位）"
        )
    ...
    if flag == "design_reviewed":
        return gate_reject(
            "design_reviewed 只能由对抗评审 PASS（review --result PASS）自动置位；"
            "禁止用 gate 命令绕过对抗评审"
        )
    value = args.value
...
# ---------------------------------------------------------------------------
# confirm（人工确认门禁专用：requirement_confirmed/plan_confirmed/test_passed/mr_reviewed/merged）
# ---------------------------------------------------------------------------
...
def cmd_close(args) -> int:
    ...
    if guard is not None:
        return guard
    # H4: 只有处于「交付上线」（交付阶段）才允许 close，防从任意状态直接闭环
    if state["main_state"] != "交付上线":
        return gate_reject(
            f"当前主状态「{state['main_state']}」，仅「交付上线」可 close（须先完成 MR/人工 review/合并部署）"
        )
    if args.conclusion:
        cp = Path(args.conclusion)
        if not cp.is_absolute():
            cp = req_dir(state["req_id"], args.dir) / cp
        if not check_file_exists(cp):
            return gate_reject(f"交付说明.md（或终稿 XX需求-REQ-SEQ需求实现说明.md）不存在或为空：{cp}")
        # H3: 交付说明可能位于 npp 仓库根（runtime/req 之外），存完整路径而非文件名
    ...
    save_state(state, args.dir)
    print(f"已关闭：{state['req_id']}，状态机闭环（主状态=结束）")
    print(progress_banner(state))
    return 0
...
def cmd_reset(args) -> int:
    """人工接管（评审 3 轮 FAIL）经人工复核后，复位 human_takeover/escalated，回到「需求分析·现状调研」。"""
    state = load_state(args.req_id, args.dir)
    if state is None:
        return fatal(f"状态文件不存在：{state_path(args.req_id, args.dir)}（先 init）")
    ...
    state["main_state"] = "需求分析"
    state["micro_state"] = REVIEW_FAIL_BACK_TO
    append_timeline(state, "reset", "需求分析", REVIEW_FAIL_BACK_TO, "人工复核后复位，重新调研")
    save_state(state, args.dir)
    print(f"已复位人工接管：{state['req_id']} → 主状态「需求分析」/微观状态「{REVIEW_FAIL_BACK_TO}」，可重新调研")
    print(progress_banner(state))
    return 0
...
def cmd_progress(args) -> int:
    ...
    同时刷新 <req-dir>/progress.md 进度卡片（旧状态文件首次查看时补齐）。"""
    state = load_state(args.req_id, args.dir)
    if state is None:
        return fatal(f"状态文件不存在：{state_path(args.req_id, args.dir)}（先 init）")
    try:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="NF 需求开发状态机引擎")
    sub = parser.add_subparsers(dest="cmd", required=True)

    p = sub.add_parser("init", help="初始化 需求单状态机")
    ...
    p = sub.add_parser("confirm", help="人工确认门禁置位（requirement_confirmed/plan_confirmed/test_passed/mr_reviewed/merged）")
    p.add_argument("--req-id", required=True)
    p.add_argument("--flag", required=True)
