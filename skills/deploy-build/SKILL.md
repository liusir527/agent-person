...
检查是否存在配置文件 `E:\agent_assets\.dsh\env_config\deploy_build\deploy_config.json`（env_config 创建基准固定为 `E:\agent_assets`，`AGENT_ASSETS_DIR` 环境变量可覆盖）。
...
配置将保存到 `E:\agent_assets\.dsh\env_config\deploy_build\deploy_config.json`（编译服务器）+ `device_config.json`（目标设备，`devices` 数组，支持多台）。日志和同步清单也统一在该目录下管理。
...
1. 向用户报告结果，包括日志文件路径（位于 `E:\agent_assets\.dsh\env_config\deploy_build\logs\deploy.log`，每次运行覆盖，只保留最新）。
2. 如编译失败，分析错误输出并协助诊断。
