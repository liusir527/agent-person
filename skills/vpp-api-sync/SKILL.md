...
- `<项目根>` = `E:\agent_assets`（**不相对当前工作目录、不相对技能所在目录**）
...
> 其他环境请用 `--remote-*`/`--arch` 覆盖。密码从 `E:\agent_assets\.dsh\env_config\deploy_build\deploy_config.json` 的
> `remote_pass` 读取（该目录不入库），或经环境变量 `DEPLOY_BUILD_REMOTE_PASS`/`SSH_PASSWORD` 注入；
> 不通过 CLI 传递。
