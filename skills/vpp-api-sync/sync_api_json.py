...
# env_config 创建基准目录：固定为 AGENT_ASSETS_DIR（默认 E:\agent_assets），不随脚本位置漂移；
# 与 deploy-build 的约定一致（AGENT_ASSETS_DIR 环境变量可覆盖）。
...
def _load_remote_pass_from_env_config():
    """从 REDACTED_WORKSPACE_PATH/.dsh/env_config/deploy_build/deploy_config.json 读取 remote_pass（该目录不入库）。
